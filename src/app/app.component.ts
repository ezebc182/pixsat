...
export class AppComponent implements OnInit, OnDestroy {
	...
	syncPosition() {
			if (this.storageService.exists(POSITION_KEY, STORAGE)) {
				const lastKnownPosition = <Satellite>this.storageService.get(POSITION_KEY, STORAGE);
	...
	private checkUserPreferences() {
		const userSettings = <UserPreferences>this.storageService.get(USER_KEY, localStorage);
			if (userSettings) {
